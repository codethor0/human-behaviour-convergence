...
def test_ambiguous_washington():
    ...
    # Verify
    assert result.normalized_location is None, "❌ Should NOT create normalized_location for ambiguous case"
    assert result.best_guess_region_id == "us_wa", f"❌ Expected 'us_wa', got '{result.best_guess_region_id}'"
    assert result.alternate_region_ids == ["us_dc"], f"❌ Expected ['us_dc'], got {result.alternate_region_ids}"
    assert result.ambiguity_reason is not None, "❌ Should have ambiguity_reason"
    assert "Ambiguous" in result.ambiguity_reason, f"❌ Reason should mention 'Ambiguous'"
...
def test_dc_context():
    ...
    if result.normalized_location:
        print(f"  normalized_location.region_id: {result.normalized_location.region_id}")
        print(f"  normalized_location.region_label: {result.normalized_location.region_label}")
    else:
        print(f"  normalized_location: None")
    
    assert result.normalized_location is not None, "❌ Should create normalized_location for DC context"
    assert result.normalized_location.region_id == "us_dc", f"❌ Expected 'us_dc', got '{result.normalized_location.region_id}'"
...
def test_wa_state_context():
    ...
    assert result.normalized_location is not None, "❌ Should create normalized_location for WA state context"
    assert result.normalized_location.region_id == "us_wa", f"❌ Expected 'us_wa', got '{result.normalized_location.region_id}'"
...
def test_forecast_config_ambiguity():
    ...
    if config.normalized_location:
        print(f"  normalized_location.region_id: {config.normalized_location.region_id}")
        print(f"  normalized_location.best_guess: {config.normalized_location.best_guess}")
        print(f"  normalized_location.alternatives: {config.normalized_location.alternatives}")
    
    assert config.normalized_location is not None, "❌ ForecastConfigBuilder should create normalized_location"
    assert config.normalized_location.best_guess is True, "❌ Should have best_guess=True for ambiguous case"
    assert len(config.normalized_location.alternatives) > 0, "❌ Should have alternatives"
    assert config.normalized_location.region_id in ["us_wa", "us_dc"], f"❌ region_id should be us_wa or us_dc"

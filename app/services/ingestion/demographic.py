...
class DemographicFetcher:
    ...
    def fetch_demographic_stress_index(
        ...
        try:
            ...
            logger.info(
                "Fetching Census demographic data", state=state_code, year=acs_year
            )
            ...
            pop_idx = (
                headers.index(CENSUS_VARIABLES["population"])
                if CENSUS_VARIABLES["population"] in headers
                else None
            )
            if pop_idx is None:
            ...
            logger.error(
                "Census API error", state=state_code, error=str(e), exc_info=True
            )
            return self._fallback_demographic_data(state_code)

...
def compute_mape(actual: pd.Series, predicted: pd.Series) -> float:
    ...
    return float(
        np.mean(
            np.abs(
                (aligned.loc[mask, "actual"] - aligned.loc[mask, "predicted"])
                / aligned.loc[mask, "actual"]
            )
        )
        * 100
    )
...
def compute_smape(actual: pd.Series, predicted: pd.Series) -> float:
    ...
    return float(
        np.mean(
            np.abs(aligned.loc[mask, "actual"] - aligned.loc[mask, "predicted"])
...
def compute_interval_coverage(
    ...
    aligned = pd.DataFrame({"actual": actual, "lower": lower, "upper": upper}).dropna()

    if len(aligned) == 0:
        return float("nan"), float("nan")

    # Coverage: percentage of actuals within interval
    in_interval = (aligned["actual"] >= aligned["lower"]) & (
        aligned["actual"] <= aligned["upper"]
    )
...
class ModelEvaluator:
    ...
    def evaluate(
        ...
        if history.empty or forecast.empty:
            logger.warning(
                "Empty history or forecast", model=model_name, region=region_id
            )
            return {
                "mae": float("nan"),
                "rmse": float("nan"),
        ...
        if not overlap:
            logger.warning(
                "No overlapping timestamps for evaluation",
                model=model_name,
                region=region_id,
            )
            return {
        ...
        history_aligned = history.set_index("timestamp").loc[
            list(overlap), "behavior_index"
        ]
        forecast_aligned = forecast.set_index("timestamp").loc[
            list(overlap), "prediction"
        ]
        lower_aligned = forecast.set_index("timestamp").loc[
            list(overlap), "lower_bound"
        ]
        upper_aligned = forecast.set_index("timestamp").loc[
            list(overlap), "upper_bound"
        ]
        ...
        coverage, avg_width = compute_interval_coverage(
            history_aligned, lower_aligned, upper_aligned
        )
    ...
    def rolling_origin_evaluation(
        ...
        window_history = full_history[
            (full_history["timestamp"] >= start_date)
            & (full_history["timestamp"] <= end_date)
        ]

        if len(window_history) < forecast_horizon:
            logger.warning(
                "Insufficient history for rolling evaluation",
                model=model_name,
                region=region_id,
            )
            return []

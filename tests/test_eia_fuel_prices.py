...
def test_eia_fuel_regional_variance(fetcher):
    ...
    try:
        ...
        assert il_mean != az_mean, f"IL ({il_mean:.4f}) and AZ ({az_mean:.4f}) should have different fuel stress indices"
        ...
        assert il_std > 0.01, f"IL fuel_stress_index should have variance (std={il_std:.6f})"
        assert az_std > 0.01, f"AZ fuel_stress_index should have variance (std={az_std:.6f})"
    finally:
        os.environ.pop("HBC_CI_OFFLINE_DATA", None)

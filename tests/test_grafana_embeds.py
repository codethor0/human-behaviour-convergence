...
def test_frontend_embed_list_matches_manifest():
    ...
    for uid in expected_dashboards:
        if (
            f'dashboardUid="{uid}"' not in content
            and f"dashboardUid='{uid}'" not in content
        ):
            missing.append(uid)
...
def test_grafana_embedding_config_static():
    ...
    assert (
        "GF_SECURITY_ALLOW_EMBEDDING=true" in content
    ), "Grafana embedding not enabled"
    assert (
        "GF_AUTH_ANONYMOUS_ENABLED=true" in content
    ), "Grafana anonymous access not enabled"

        ...
        with open(file_path, "r") as f:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if match:
                    ...
                    context_start = max(
                        0, i - 10
                    )  # Look further back for region_key assignment
...
def classify_cache_key(
    pattern: str, source_id: str, context: str = ""
) -> Dict[str, any]:
    """
    ...
    if "region_key" in pattern.lower() or re.search(
        r"region_key\s*=", context, re.IGNORECASE
    ):
    ...
    if is_regional_source and result["is_global_cache"]:
        result["issue"] = (
            f"REGIONAL source '{source_id}' uses global cache key (no geo params)"
        )

...
def compute_variance_metrics(forecasts: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for forecast in forecasts:
        ...
        for parent in ["economic_stress", "environmental_stress", "political_stress", "mobility_activity"]:
    ...
    for field in ["history", "forecast", "behavior_index", "sub_indices", 
                  "parent_economic_stress", "parent_environmental_stress", 
                  "parent_political_stress", "parent_mobility_activity"]:
        unique_values = set()
        ...
        results["variance_matrix"].append({
            ...
            "total_regions": len(forecasts),
            "variance_ratio": len(unique_values) / len(forecasts) if forecasts else 0,
        })
...
def main():
    """Main execution."""
    output_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/hbc_variance_harness")
    ...
    for region in TEST_REGIONS:
        try:
            ...
            forecast_file = output_dir / f"forecast_{region['name'].lower().replace(' ', '_')}.json"
    ...
    for row in variance_results["variance_matrix"]:
        ...
        print(f"  {status} {row['field']}: {row['unique_count']}/{row['total_regions']} unique ({row['variance_ratio']:.2%})")
    
    # Check for P0 issues (too many identical hashes)
    p0_fields = [r for r in variance_results["variance_matrix"] 
